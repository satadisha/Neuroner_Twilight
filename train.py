...
def prediction_step(sess, dataset, dataset_type, model, transition_params_trained,
    ...
    output_filepath = os.path.join(stats_graph_folder, '{1:03d}_{0}.txt'.format(dataset_type,
        epoch_number))
    mentions_output_filepath = os.path.join(stats_graph_folder,'mentions_output.txt')
    ...
    print('tally:', len(token_list_outer), len(mentions_list_outer),'total mentions discovered:', mention_count)
